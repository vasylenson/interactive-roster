...
import { names, Schedule, Week, type Person } from '../../library/schedule';
...
export default function Home() {
    ...
    const generate = () => {
        const n = weeks();
        if (n === null) return [];
        const newRows = Array.from(schedule.formattedAssignments().take(n));
        ...
        console.log(
            Array.from(
                schedule
                    .assignments()
                    .map(([date, tasks]) => [Week.from(date).id, tasks])
                    .take(14)
            )
        );
    };
